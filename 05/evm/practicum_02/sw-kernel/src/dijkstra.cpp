...
void create_visualization() {
    ...
    uint32_t max_x = mq_receive();
    uint32_t max_y = mq_receive();

    matrix_layout(G,vG,max_x,max_y);
...
void insert_edges () {
    ...
    for (unsigned int i = buffer_pointer; i < buffer_pointer + size; i += 3 * sizeof(int)) {
        ...
        unsigned int    w = *((volatile unsigned int*)(AXI4EXTMEM_BASE + i + 2 * sizeof(int)));
        if (v != u) {
            //u->v edge
            ...
            if (record) {
                ...
                if (!flag) {
                    G.ins_sync(Graph::Key{.index = (unsigned int)record.value().adj_c, .u = u}, Graph::Edge{.v = v, .w = w});
                ...
                G.ins_sync(Graph::Key{.index = 0, .u = u}, Graph::Edge{.v = v, .w = w});
...
void dijkstra_core(unsigned int start_vertex) {
    ...
    while(auto q_it = Q.begin()) {
        ...
        //For each Adj
        for (auto [adj, wu] : edge_range(G, u)) {
