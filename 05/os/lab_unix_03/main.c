...
void daemonize(const char* cmd)
{
    ...
    if ((pid = fork()) == -1) 
    {
        syslog(LOG_ERR, "%s: fork error", cmd);
        exit(1);
    }
    else if (pid > 0)
        exit(0);
    ...
    {
        syslog(LOG_ERR, "%s: ignore SIGHUP error", cmd);
        exit(1);
    }
    ...
    if (chdir("/") == -1)
    {
        syslog(LOG_ERR, "%s: change directory error", cmd);
        exit(1);
    }

    if (getrlimit(RLIMIT_NOFILE, &rl) == -1)
    {
        syslog(LOG_ERR, "%s: file limit error", cmd);
        exit(1);
    }
