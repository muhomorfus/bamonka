...
valgrind --quiet --leak-check=full --errors-for-leak-kinds=all --error-exitcode=$valgrind_code --log-file="$(printf "../out/memcheck_neg_%02d.log" "${test}")" ../app.exe < "$test_input" > "$program_output"
