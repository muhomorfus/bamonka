...
/*
 * get_bit_normal получения n-ого бита числа.
 * get_bit_normal(00...001010, 0) = 0.
 * get_bit_normal(00...001010, 1) = 1.
 * get_bit_normal(00...001010, 2) = 0.
 * get_bit_normal(00...001010, 3) = 1.
 */
block_t get_bit_normal(block_t b, int n) {
    return (b >> n) & 1;
}

/*
 * get_bit получение бита числа, если считать номер бита с единицы, и отсчитывать справа.
 */
block_t get_bit(block_t b, int n, int size) {
    return (b >> (size - n)) & 1;
}
...
block_t ip(des_t des, block_t block) {
    ...
    for (int i = 1; i <= SIZE; i++) {
        result = set_bit(result, get_bit(block, des.ip[i - 1], SIZE), i, SIZE);
    }
...
block_t ip_reversed(des_t des, block_t block) {
    ...
    for (int i = 1; i <= SIZE; i++) {
        result = set_bit(result, get_bit(block, des.ip_reversed[i - 1], SIZE), i, SIZE);
    }
...
block_t g(des_t des, block_t key) {
    ...
    for (int i = 1; i <= KEY_SIZE; i++) {
        result = set_bit(result, get_bit(key, des.g[i - 1], SIZE), i, KEY_SIZE);
    }
...
block_t h(des_t des, block_t key) {
    block_t cd = 0b11100001100110010101010111111010101011001100111100011110;

    block_t result = 0;

    for (int i = 1; i <= KEY_COMPRESSED_SIZE; i++) {
        result = set_bit(result, get_bit(key, des.h[i - 1], KEY_SIZE), i, KEY_COMPRESSED_SIZE);
    }
...
block_t p(des_t des, block_t r) {
    ...
    for (int i = 1; i <= HALF_SIZE; i++) {
        result = set_bit(result, get_bit(r, des.p[i - 1], HALF_SIZE), i, HALF_SIZE);
    }
...
block_t e(des_t des, block_t r) {
    ...
    for (int i = 1; i <= KEY_COMPRESSED_SIZE; i++) {
        result = set_bit(result, get_bit(r, des.e[i - 1], HALF_SIZE), i, KEY_COMPRESSED_SIZE);
    }
...
block_t f(des_t des, block_t r, block_t k) {
    ...
    for (int i = 8; i > 0; i--) {
        block_t row = get_bit_normal(b[i - 1], 5) << 1 | get_bit_normal(b[i - 1], 0);
 ...
 *
 * Затем к полученной параше справа добавляем то, что вышло за границу:
